import { CourseElement } from "@/store/courseStore";
import { getMarkerSvg } from "./markers/markerIcons";

type OnSelect = (id: string) => void;
type OnDragEnd = (id: string, lat: number, lng: number) => void;

export class MarkerOverlay extends google.maps.OverlayView {
  private element: CourseElement;
  private isSelected: boolean;
  private onSelect: OnSelect;
  private onDragEnd: OnDragEnd;
  private div: HTMLDivElement | null = null;
  private dragging = false;
  private dragOffset = { x: 0, y: 0 };

  constructor(
    element: CourseElement,
    isSelected: boolean,
    onSelect: OnSelect,
    onDragEnd: OnDragEnd
  ) {
    super();
    this.element = element;
    this.isSelected = isSelected;
    this.onSelect = onSelect;
    this.onDragEnd = onDragEnd;
  }

  onAdd() {
    const div = document.createElement("div");
    div.style.position = "absolute";
    div.style.userSelect = "none";
    div.style.cursor = "grab";
    div.style.transform = "translate(-50%, -50%)";
    div.innerHTML = getMarkerSvg(this.element.type, this.isSelected);

    if (this.element.label) {
      const label = document.createElement("div");
      label.style.cssText =
        "position:absolute;top:100%;left:50%;transform:translateX(-50%);white-space:nowrap;font-size:11px;background:rgba(255,255,255,0.85);padding:1px 4px;border-radius:3px;pointer-events:none;margin-top:2px;";
      label.textContent = this.element.label;
      div.appendChild(label);
    }

    // Click to select
    div.addEventListener("click", (e) => {
      e.stopPropagation();
      this.onSelect(this.element.id);
    });

    // Drag
    div.addEventListener("mousedown", (e) => {
      if (e.button !== 0) return;
      e.stopPropagation();
      this.dragging = true;
      this.div!.style.cursor = "grabbing";
      const panes = this.getPanes();
      if (panes) {
        const rect = this.div!.getBoundingClientRect();
        this.dragOffset = { x: e.clientX - rect.left, y: e.clientY - rect.top };
      }

      const onMouseMove = (me: MouseEvent) => {
        if (!this.dragging || !this.div) return;
        const map = this.getMap() as google.maps.Map;
        const projection = this.getProjection();
        const mapDiv = (map as unknown as { getDiv: () => HTMLElement }).getDiv();
        const mapRect = mapDiv.getBoundingClientRect();
        const px = me.clientX - mapRect.left;
        const py = me.clientY - mapRect.top;
        const latLng = projection.fromContainerPixelToLatLng(
          new google.maps.Point(px, py)
        );
        if (latLng) {
          this.element = { ...this.element, lat: latLng.lat(), lng: latLng.lng() };
          this.draw();
        }
      };

      const onMouseUp = () => {
        this.dragging = false;
        if (this.div) this.div.style.cursor = "grab";
        document.removeEventListener("mousemove", onMouseMove);
        document.removeEventListener("mouseup", onMouseUp);
        this.onDragEnd(this.element.id, this.element.lat, this.element.lng);
      };

      document.addEventListener("mousemove", onMouseMove);
      document.addEventListener("mouseup", onMouseUp);
    });

    this.div = div;
    const panes = this.getPanes();
    panes?.overlayMouseTarget.appendChild(div);
  }

  draw() {
    if (!this.div) return;
    const projection = this.getProjection();
    const point = projection.fromLatLngToDivPixel(
      new google.maps.LatLng(this.element.lat, this.element.lng)
    );
    if (point) {
      this.div.style.left = `${point.x}px`;
      this.div.style.top = `${point.y}px`;
    }
  }

  onRemove() {
    this.div?.parentNode?.removeChild(this.div);
    this.div = null;
  }
}
